...
be implemented shabbily as analog or properly as digital. _On the
other hand, a brute force parallel search beats them all!_ The oracle
of Grover’s algorithm is a brute force parallel search. You might as
well just use its output. Furthermore, the analysis of “quantum”
algorithms uniformly neglects to account for the speed of propagation
of signals, always assuming signals propagate “instantaneously,” which
is impossible. Analysis of digital algorithms, on the other hand,
accounts for the clock frequency.
